...
int insertarArbol(tArbol* p, const void* d, unsigned tam, int(* cmp)(const void*, const void*))
{
    ...
    while(*p)
    {
        if(*p && (comp = cmp(d, (*p)->info)) > 0)
            p = &(*p)->der;

        else if(*p && (comp = cmp(d, (*p)->info)) < 0)
            p = &(*p)->izq;
...
int eliminarElemArbol(tArbol* p, void* d, unsigned tam, int(* cmp)(const void*, const void*))
{
    ...
    return eliminarRaizArbol(p, d, tam, cmp);
}
...
int cargarArchivoBinOrdenadoAbiertoArbol(tArbol* p, FILE* pf, unsigned tamInfo)
{
    ...
    cargarDatosDesdeArch(p, pf, tamInfo, 0, cantReg - 1, (void*)copiarDatoArch);
                         ...
                         int(*accion)(void*, void*, unsigned, int))
{
    ...
    *p = nue;
    ...
    int r = cargarDatosDesdeArch(&(*p)->izq, orig, tam, li, media-1, accion);
